...
def collect_mysql_data(service: str, timeout: int = 30) -> dict[str, Any]:
    ...
    batch2_query = (
        QUERY_TABLE_SIZES + "; " + QUERY_PROCESSLIST + "; " + QUERY_TOP_QUERIES
    )
...
def parse_mysql_data(data: dict[str, Any], result: MySQLAnalysisResult) -> None:
    ...
    connection_usage_pct = (
        round(max_used_connections / max_connections * 100, 1)
    ...
    tmp_disk_pct = (
        round(created_tmp_disk / created_tmp * 100, 1) if created_tmp > 0 else 0
    )
    ...
    table_scan_pct = (
        round((handler_rnd_next + handler_first) / scan_total * 100, 1)
        if scan_total > 0
        else 0
    )
    ...
    hit_ratio = (
        round((read_requests - reads) / read_requests * 100, 2)
        if read_requests > 0
        else 0
    )
    ...
    table_lock_contention = (
        round(table_locks_waited / lock_total * 100, 2) if lock_total > 0 else 0
    )
...
def _status_ok_warn_crit(
    value: float, warn_threshold: float, crit_threshold: float
) -> str:
...
def generate_recommendations(result: MySQLAnalysisResult) -> list[dict[str, str]]:
    ...
    if conn_pct >= 90:
        ...
        rec(
            "warning",
            f"Connection usage at {conn_pct}%. Consider increasing max_connections.",
        )
    ...
    if hit_ratio < 95:
        rec(
            "critical",
            f"Buffer pool hit ratio at {hit_ratio}%. Increase innodb_buffer_pool_size.",
        )
    elif hit_ratio < 99:
        rec(
            "warning",
    ...
    if bp_usage > 95:
        rec(
            "warning",
            f"Buffer pool {bp_usage}% full. Data pages may be evicted under load.",
        )
    ...
    if result.top_queries:
        no_index_count = sum(
            1 for q in result.top_queries if q.get("no_index_used", 0) > 0
        )
        if no_index_count > 0:
            rec(
                "warning",
...
def format_report(result: MySQLAnalysisResult) -> str:
    ...
    if ov:
        ...
        table_row(
            "Aborted Clients", _format_count(ov["aborted_clients"]), aborted_c_status
        )
        aborted_conn_status = "WARN" if ov["aborted_connects"] > 0 else "OK"
        table_row(
            "Aborted Connects",
            _format_count(ov["aborted_connects"]),
            aborted_conn_status,
        )
    ...
    if bp:
        ...
        table_row("Metric", "Value", "Status")
        table_sep(3)
        hr_status = _status_ok_warn_crit(
            100 - bp["hit_ratio"], 1, 5
        )  # inverted: lower hit = worse
    ...
    if result.top_queries:
        ...
        if result.top_queries_status == "performance_schema_disabled":
            lines.append(
                "performance_schema is disabled — no query-level data available."
            )
            lines.append(
            ...
            lines.append(
                "No queries recorded. Database may be idle or recently restarted."
            )
        else:
...
def analyze_mysql(
    ...
    def task_mysql_queries():
        if not ssh_available:
            return {
                "errors": [f"SSH not available: {ssh_stderr or 'connection failed'}"]
            }
