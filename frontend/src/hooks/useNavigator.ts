// The intake conversation over /api/navigator: jev answers from the
// description, the rules engine picks the questions, the user fills the gaps.

import { useCallback, useRef, useState } from "react";
import { navigatorReply, navigatorStart } from "../lib/api.ts";
...
export function useNavigator({ onError }: { onError: (err: unknown) => void }) {
  ...
  const canSend = !thinking && !ended && !result && (!sessionId || !!question);
